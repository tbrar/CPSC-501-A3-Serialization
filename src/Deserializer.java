...
public class Deserializer {
	...
	public void deserialize(Document doc) {
		...
		for(int i = 0; i < elements.size(); i++ ){
			...
			if(c.isArray()) {
				obj = deserializeArray(cl, c);
			}
			else if(Collection.class.isAssignableFrom(c)) {
				obj = deserializeCollection(cl, c);
			}
			else {
				deserializeObject(cl, c);
			}
	...
	private void deserializeObject(Element cl, Class c) {
		Constructor ctr = null;
		Object obj = null;
		try {
	...
	private Object deserializeCollection(Element cl, Class c) {
		Constructor ctr = null;
		Object obj = null;
		try {
			ctr = c.getDeclaredConstructor();
	...
	private Object deserializeArray(Element cl, Class c) {
		Object obj;
		...
		for(int j = 0; j< vals.size(); j++) {
			...
			if(c.getComponentType().isPrimitive()) {
				deserializePrimative(c, obj, j, val);
			}
			else {
				Object objval = hash.get(val);
	...
	private void deserializePrimative(Class c, Object obj, int index, String val) {
		if(c.getComponentType().equals(byte.class)) {
			Array.setByte(obj, index, Byte.valueOf(val));
		}
		else if(c.getComponentType().equals(short.class)) {
			Array.setShort(obj, index, Short.valueOf(val));
		}
		else if(c.getComponentType().equals(int.class)) {
			Array.setInt(obj, index, Integer.valueOf(val));
		}
		else if(c.getComponentType().equals(long.class)) {
			Array.setLong(obj, index, Long.valueOf(val));
		}
		else if(c.getComponentType().equals(float.class)) {
			Array.setFloat(obj, index, Float.valueOf(val));
		}
		else if(c.getComponentType().equals(double.class)) {
			Array.setDouble(obj, index, Double.valueOf(val));
		}
		else if(c.getComponentType().equals(char.class)) {
			Array.setChar(obj, index, val.charAt(0));
		}
		else if(c.getComponentType().equals(boolean.class)) {
			Array.setBoolean(obj, index, Boolean.valueOf(val));
		}
	}
